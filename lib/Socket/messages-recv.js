...
const Long = require('long').default || require('long');
const { proto } = require('../../WAProto/index.js');
const { DEFAULT_CACHE_TTLS, KEY_BUNDLE_TYPE, MIN_PREKEY_COUNT, PLACEHOLDER_MAX_AGE_SECONDS, STATUS_EXPIRY_SECONDS } = require('../Defaults/index.js');
const { ReachoutTimelockEnforcementType, WAMessageStatus, WAMessageStubType } = require('../Types/index.js');
const { ACCOUNT_RESTRICTED_TEXT, aesDecryptCTR, aesEncryptGCM, cleanMessage, Curve, decodeMediaRetryNode, decodeMessageNode, decryptMessageNode, delay, derivePairingCodeKey, encodeBigEndian, encodeSignedDeviceIdentity, extractAddressingContext, extractE2ESessionFromRetryReceipt, getCallStatusFromNode, getHistoryMsg, getNextPreKeys, getStatusFromReceiptType, handleIdentityChange, hkdf, MISSING_KEYS_ERROR_TEXT, NACK_REASONS, NO_MESSAGE_FOUND_ERROR_TEXT, SERVER_ERROR_CODES, toNumber, unixTimestampSeconds, xmppPreKey, xmppSignedPreKey, generateWAMessageFromContent } = require('../Utils/index.js');
const { makeMutex } = require('../Utils/make-mutex.js');
const { makeOfflineNodeProcessor } = require('../Utils/offline-node-processor.js');
const { buildAckStanza } = require('../Utils/stanza-ack.js');
const { buildMergedTcTokenIndexWrite, isTcTokenExpired, readTcTokenIndex, resolveIssuanceJid, resolveTcTokenJid, storeTcTokensFromIqResult, TC_TOKEN_INDEX_KEY } = require('../Utils/tc-token-utils.js');
const { areJidsSameUser, binaryNodeToString, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildBuffer, getBinaryNodeChildren, getBinaryNodeChildString, getBinaryNodeChildUInt, isJidGroup, isJidNewsletter, isJidStatusBroadcast, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, S_WHATSAPP_NET } = require('../WABinary/index.js');
const { extractGroupMetadata } = require('./groups.js');
const { makeMessagesSocket } = require('./messages-send.js');
...
const makeMessagesRecvSocket = (config) => {
    ...
    const offerCall = async (toJid, isVideo = false) => {
        ...
        const { nodes: destinations, shouldIncludeDeviceIdentity } =
            await createParticipantNodes(devices, { call: { callKey: encKey } });
